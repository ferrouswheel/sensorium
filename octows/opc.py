...
def handle_pixel(sock, address):
    fp = sock.makefile()
    while True:
        channel, _, pixel_length = unpack('>BBh', fp.read(4))
        pixel_data = fp.read(pixel_length)
        for pixel_color in zip(*(pixel_data[i::3] for i in range(3))):
            r, g, b = unpack('>BBB', ''.join(pixel_color))
...
class Server(object):
    ...
    def __init__(self, server_ip_port, verbose=False):
